...
**수정** ([run_pipeline.py](../../../tools/capcut_pipeline/run_pipeline.py) `wrap_segments` `tok_score >= 3` 블록): 조사/연결어미(3~7점)에서 break 후보일 때, **max_chars 내에 더 좋은 break(쉼표/종결 8+)가 도달 가능하면 거기까지 보류**.
...
자막 교정 단계에서 **텍스트 맞춤법뿐 아니라 줄바꿈(wrap) 끊김도 육안 점검**할 것. ⛔ **맞춤법만 보고 "자연스럽다"고 넘기지 말 것** — 자동 wrap(글자수 균등 / Scribe word 단위)은 의미 경계를 자주 깬다. **모든 cue 경계를 하나씩 읽으며 확인**:

| 깨지는 패턴 | 예시 (잘못된 끊김) |
|---|---|
| 관형형 어미 + 의존명사 | `변하고 있는` / `건지` · `찾아주는` / `건` |
| 주어부 + 서술부 | `내가 제대로` / `이끌어내고 있나` |
| 관형구 + 명사 | `강점이라는` / `도구가` · `사람을 강점으로` / `보기` |
| 수사 + 단위 | `30만` / `명의` · `3` / `가지` |

끊김이 깨졌으면 재wrap 필요. 재wrap은 cue 수를 바꾸므로 **불변 조건이 아닌 재처리 체인**을 탄다.

**방법 A — 자동 재wrap** (가벼운 경우):
```bash
...
# → 결과를 다시 육안 점검. 여전히 깨지면 방법 B.
```

**방법 B — word-level 수동 재wrap** ⭐ (자동이 부족할 때 — 가장 확실. 2026-06-01 실측: 자동 wrap이 이미 깨져 수동으로 해결):
1. `output/<name>/subs/transcript.json`의 word-level `start/end/word`를 Read
2. word를 **의미 단위(구·절)로 직접 묶어** cue 경계 결정 (≤18~20자, 위 표의 패턴 절대 분리 금지)
3. 각 cue `start`=첫 word.start, `end`=끝 word.end, 텍스트는 맞춤법·숫자 교정 반영
4. `transcript_wrapped.srt` + `transcript_wrapped.raw.srt` **동일 내용**으로 Write (raw도 같이 = verify_step 2 통과)
```python
# 패턴: CUES=[(start_word_idx, end_word_idx_inclusive, "교정텍스트"), ...]
# st=W[a]['start']; en=W[b]['end']; SRT 포맷 HH:MM:SS,mmm 으로 Write
```

**공통 마무리 (A·B 모두)**:
```bash
# 드래프트 재빌드: /capcut <video> --skip-stt --skip-wrap --skip-cut --sub-offset-ms 300 (Scribe) / 600 (Whisper)
# overlay_patcher + capcut_fx_patcher 재패치 (clean_bak 주의 → BGM/폰트/timing 재확정)
```
...
| **영문 약어 한글표기** ⛔ | `에이아이` → **`AI`** · `디엔에이` → **`DNA`** · `에이피아이` → `API` · `유아이` → `UI` · `시이오` → `CEO` 등. STT가 영문 약어를 한글 음차로 받아쓰면 **반드시 영문 대문자로 복원** (조사 유지: `에이아이가`→`AI가`, `디엔에이로`→`DNA로`) |

**판단 기준**: 오디오가 어떻게 들렸을지 추론해서 원 발화에 **가장 가까운 표준 표기**로 복원. 말투 자체는 건드리지 않음.

> ⛔ **영문 약어는 항상 영문 표기 (사용자 강한 선호, 2026-06-05)**: STT(Scribe/Whisper)가 `AI`를 `에이아이`로, `DNA`를 `디엔에이`로, `API`를 `에이피아이`로 음차하는 일이 잦다. **Step 2 교정 시 `에이아이`→`AI`, `디엔에이`→`DNA` 등 전수 치환 필수** (자막·script.txt·emphasis·title 모두). 글자수 wrap에도 영향(`에이아이가` 4자 → `AI가` 3자)이지만 cue 경계/타임스탬프는 불변 — 텍스트만 치환하면 verify_step 2 통과. 사용자가 "자꾸 에이아이/디엔에이라고 표현해서 귀찮다"고 명시 — 재발 금지.
...
- [tools/capcut_pipeline/build_capcut_draft.py](../../../tools/capcut_pipeline/build_capcut_draft.py) — 드래프트 빌더 (4패치 호출)
- [tools/capcut_pipeline/run_stt.py](../../../tools/capcut_pipeline/run_stt.py) — Whisper STT + SRT wrap
